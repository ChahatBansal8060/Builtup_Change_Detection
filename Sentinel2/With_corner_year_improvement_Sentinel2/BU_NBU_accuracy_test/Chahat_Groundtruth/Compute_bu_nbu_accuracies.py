...
'''
def Get_overall_accuracy(districts, tiffiles_folder, pixelType_dict, year):
    actual_output = []
    predicted_output = []
    
    for district in districts:    
        for pixel_type in pixelType_dict.keys():
            tif_files_path = tiffiles_folder+'/'+district+'/'+year+'/'+district+'_'+pixel_type
    ...
    print(classification_report(actual_output, predicted_output, target_names=['CBU', 'CNBU'], digits=4))
...
This function computes the overall accuracy across all districts
'''
def Get_district_wise_accuracy(districts, tiffiles_folder, pixelType_dict, year):
...
for year in years:
    ...
    Get_overall_accuracy(districts, tiffiles_folder, pixelType_dict, year)
    Get_district_wise_accuracy(districts, tiffiles_folder, pixelType_dict, year)
